...
const DropTarget = observer(({ onDrop, className }: DropProps) => {
    ...
    const handleDragIn = (e: DragEvent) => {
        ...
        if (
            ...
            find(e.dataTransfer.items, (it: DataTransferItem) => it.kind == 'file')
        )
            setActive(true);
    };
    ...
    return (
        <DropTargetContainer ref={dropTarget} $isActive={isActive} className={className}>
            {isActive && (
                <DropTargetContent>
                    <DropTargetText>Drop to parse</DropTargetText>
                </DropTargetContent>
            )}
        </DropTargetContainer>
    );
