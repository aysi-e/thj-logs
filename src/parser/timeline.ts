...
export default class Timeline {
    ...
    replaceId(oldId: string, newId: string) {
        values(this.events).forEach((events) =>
            values(events).forEach((item) => {
...
export const toDPSData = (
    ...
    const startTime =
        start && start / 1000 > parseInt(first(keys(timeline.events))!)
            ? round(start / 1000)
            : parseInt(first(keys(timeline.events))!);
    // if we have an end time (and it's earlier than our timeline end time), use it.
    const endTime =
        end && end / 1000 < parseInt(last(keys(timeline.events))!)
            ? round(end / 1000)
            : parseInt(last(keys(timeline.events))!);
    // if our start/end times are bad, error out.
    if (startTime > endTime)
        throw new Error(`start time was after end time when generating dps data`);
    ...
    const damageItems = range(startTime, endTime).map((i, index) => {
        ...
        return {
            x: index,
            y: reduce(
                values(timeline.events[i]),
                (acc, val) => {
                    if (sourceIdFilter(val) && targetIdFilter(val) && nameFilter(val))
